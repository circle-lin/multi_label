...
class broadnet_enhence:
    ...
    def fit(self,oridata,orilabel):
        ...
        if (type(oridata) is np.ndarray):
            {}
        else:
            oridata = oridata.toarray()
        if (type(orilabel) is np.ndarray):
            {}
        else:
            orilabel = orilabel.toarray()

        data, X_alpha, label, y_alpha = train_test_split(oridata, orilabel, test_size=0.05)
        ...
        accuracy, i = accuracy_score(Y,orilabel),0
    ...
    def decoder_alpha(self,testdata,y_true):
